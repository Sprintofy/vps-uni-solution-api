...
import fileStorageService from "./utilities/fileStorageService"; // ADD

const fetch_all_clients_trades = async (req: any) => {
  try {
    const clients = await clientTradeModel.fetchAllClientsTradesByOrganization(
      1,
      req.body.filterData,
      req.body.query || "",
      req.body.pageSize,
      (req.body.pageIndex - 1) * req.body.pageSize,
      req.body.sort || ""
    );
    const total =
      await clientTradeModel.fetchAllClientsTradesCountByOrganization(
        1,
        req.body.filterData,
        req.body.query || ""
      );
    const client_statistics =
      await clientTradeModel.fetchAllClientsProofsStatistics(
        1,
        req.body.filterData,
        req.body.query || ""
      );
    const organization_statistics =
      await clientTradeModel.fetchAllOrganizationProofsStatistics(
        1,
        req.body.filterData,
        req.body.query || ""
      );
    const date_statistics =
      await clientTradeModel.fetchAllOrganizationDateProofsStatistics(
        1,
        req.body.filterData,
        req.body.query || ""
      );
    ...
    clients.forEach((client: any) => {
      const stats = client_statistics.find(
        (stat: any) => stat.client_id === client.client_id
      );
      if (stats) {
    ...
    return {
      total_client_count:
        (organization_statistics.length &&
          organization_statistics[0].total_client_count) ||
        0,
      total_email_sent:
        (organization_statistics.length &&
          organization_statistics[0].total_email_sent) ||
        0,
      total_email_received:
        (organization_statistics.length &&
          organization_statistics[0].total_email_received) ||
        0,
      day_total_client_count:
        (date_statistics.length && date_statistics[0].day_total_client_count) ||
        0,
      day_total_email_sent:
        (date_statistics.length && date_statistics[0].day_total_email_sent) ||
        0,
      day_total_email_received:
        (date_statistics.length &&
          date_statistics[0].day_total_email_received) ||
        0,
      data: clients,
...
const fetch_all_clients_trades_logs = async (req: any) => {
  try {
    const clients = await clientTradeModel.fetch_all_clients_trades_logs(
      req.body.client_id,
      req.body.query || "",
      req.body.pageSize,
      (req.body.pageIndex - 1) * req.body.pageSize,
    ...
    const total = await clientTradeModel.fetch_all_clients_trades_logs_count(
      req.body.client_id,
      req.body.query || ""
    );
    return {
...
const process_pre_trade_data = async (
  req: any,
  file: any,
  fields: any,
  data: any[]
  ...
  try {
    const fileLogs = await save_trade_file_log(req, {
...
const save_bulk_pre_trade_info = async (req: any, fields: any, data: any) => {
  try {
    ...
    const pre_trades_info = await add_client_info_in_pre_trade(
      req,
      fields,
      data
    );
    ...
    const preTradeInfoPromises = pre_trades_info.trade_info.map(
      async (trade: any) => {
    ...
    const batchResults = pre_trades_info.client_wise_trades.map(
      async (client: any) => {
...
const add_client_info_in_pre_trade = async (
  req: any,
  fields: any,
  data: any
) => {
  try {
    const uniqueClientCodes = [
      ...new Set(data.map((item: any) => item.client_code)),
    ];

    //console.log("uniqueClientCodes",uniqueClientCodes);

    const clients = await clientModel.fetchClientInfoByIds(
      uniqueClientCodes,
    ...
    const trade_info = data.map((order: any) => {
      const client = clients.find(
        (client: any) =>
          client.client_code.toLowerCase() === order.client_code.toLowerCase()
      );
      if (client) {
        return {
          ...order,
    ...
    const client_wise_trades = clients.map((client: any) => {
      const trades = trade_info.filter(
        (trade: any) =>
          trade.client_code.toLowerCase() === client.client_code.toLowerCase()
      );
      return {
        ...client,
...
const check_validation_pre_trade_client = async (req: any, client: any) => {
  try {
    ...
    const comparisonKeys = [
      ...
      {
        key: "trigger_price",
        normalize: (value: any) => parseFloat(value).toFixed(2),
      }, // Normalize to float with 2 decimals
    ];

    if (is_send_unique_email) {
      const existing_pre_trade =
        await clientTradeModel.fetch_existing_pre_trade_client_ids([
          client.client_id,
        ]);
      ...
      client.trade_info.forEach((trade: any) => {
        ...
        const isMatch = existing_pre_trade.some((existingTrade: any) => {
          ...
          return comparisonKeys.every(({ key, normalize }) => {
            ...
            const existingValue = normalize
              ? normalize(existingTrade[key])
              : existingTrade[key];
...
const fetch_trades_details_by_client_id = async (req: any) => {
  try {
    ...
    const trades = await clientTradeModel.fetch_trade_proof_details(
      req.query.proof_id
    );
    response.trade_info = trades;

    const trades_proofs = await clientTradeModel.fetch_trade_proof_by_client_id(
      req.query.client_id
    );
    ...
    response.trade_proof = trades_proofs.map((proof: any) => {
      const relatedTrades = trades.filter(
        (trade: any) => trade.pre_proof_id === proof.pre_trade_proof_id
      );
      return {
        ...proof,
...
// ============================================
// UPDATED: download_all_email - Using fileStorageService
// ============================================
const download_all_email = async (req: any) => {
  try {
    const file_name = `trade_all_files_${moment().format(
      "DD_MM_YYYY_HH-mm-ss"
    )}.zip`;
    ...
    // UPDATED: Use fileStorageService instead of awsS3BucketService
...
// ============================================
// UPDATED: download_all_pdf - Using fileStorageService
// ============================================
  ...
  try {
    ...
    for (const key of keys) {
      ...
      try {
        ...
        console.error(
          `Failed to download file from ${key.pdf_url}: ${downloadError.message}`
        );
      }
    }
...
// ============================================
// UPDATED: download_zip_file - Using fileStorageService
// ============================================
...
// ============================================
// UPDATED: download_all_pdf_by_client - Using fileStorageService
// ============================================
const download_all_pdf_by_client = async (req: any) => {
  try {
    ...
    const keys = await clientTradeModel.fetch_all_trade_proof_urls_by_client_id(
      req.query.client_id
    );
...
// ============================================
// UPDATED: download_all_email_by_client - Using fileStorageService
// ============================================
...
const save_pre_trade = async (req: any, body: any) => {
  const data = {
    organization_id:
      req.user && req.user.organization_id ? req.user.organization_id : 1,
  ...
  if (
    body.pre_trade_info_id !== undefined &&
    body.pre_trade_info_id !== null &&
    body.pre_trade_info_id !== ""
  )
    data.pre_trade_info_id = body.pre_trade_info_id;
  if (
    body.client_code !== undefined &&
    body.client_code !== null &&
    body.client_code !== ""
  )
    data.client_code = body.client_code;
  if (
    body.exchange_code !== undefined &&
    body.exchange_code !== null &&
    body.exchange_code !== ""
  )
    data.exchange_code = body.exchange_code;
  if (
    body.buy_or_sell !== undefined &&
    body.buy_or_sell !== null &&
    body.buy_or_sell !== ""
  )
    data.buy_or_sell = body.buy_or_sell;
  if (
  ...
  if (
    body.script_name !== undefined &&
    body.script_name !== null &&
    body.script_name !== ""
  )
    data.script_name = body.script_name;
  if (
  ...
  if (body.lots !== undefined && body.lots !== null && body.lots !== "")
    data.lots = body.lots;
  if (
  ...
  if (
    body.discounted_quantity !== undefined &&
    body.discounted_quantity !== null &&
    body.discounted_quantity !== ""
  )
    data.discounted_quantity = body.discounted_quantity;
  if (
    body.trigger_price !== undefined &&
    body.trigger_price !== null &&
    body.trigger_price !== ""
  )
    data.trigger_price = body.trigger_price;
  if (
    body.order_life !== undefined &&
    body.order_life !== null &&
    body.order_life !== ""
  )
    data.order_life = body.order_life;
  if (
    body.gtd_value !== undefined &&
    body.gtd_value !== null &&
    body.gtd_value !== ""
  )
    data.gtd_value = body.gtd_value;
  if (
    body.pre_proof_id !== undefined &&
    body.pre_proof_id !== null &&
    body.pre_proof_id !== ""
  )
    data.pre_proof_id = body.pre_proof_id;
  if (
    body.is_email_sent !== undefined &&
    body.is_email_sent !== null &&
    body.is_email_sent !== ""
  )
    data.is_email_sent = body.is_email_sent;
  if (
    body.is_email_received !== undefined &&
    body.is_email_received !== null &&
    body.is_email_received !== ""
  )
    data.is_email_received = body.is_email_received;
  if (
    body.email_url !== undefined &&
    body.email_url !== null &&
    body.email_url !== ""
  )
    data.email_url = body.email_url;
  if (
...
const update_pre_trade = async (req: any, body: any) => {
  ...
  const results = await clientTradeModel.update_pre_trade_proofs(
    data,
    body.pre_tades_proof_id
  );
  return results;
};
...
const save_pre_trade_proofs = async (req: any, body: any) => {
  ...
  if (
    body.pre_trade_id !== undefined &&
    body.pre_trade_id !== null &&
    body.pre_trade_id !== ""
  )
    data.pre_trade_id = body.pre_trade_id;
  if (
    body.client_id !== undefined &&
    body.client_id !== null &&
    body.client_id !== ""
  ...
  if (
    ...
    data.organization_id = body.organization_id;
  if (
    body.email_response !== undefined &&
    body.email_response !== null &&
    body.email_response !== ""
  )
    data.email_response = body.email_response;
  if (
    body.is_email_sent !== undefined &&
    body.is_email_sent !== null &&
    body.is_email_sent !== ""
  ...
  if (
    ...
    data.email_url = body.email_url;
  if (
    body.email_sample_url !== undefined &&
    body.email_sample_url !== null &&
    body.email_sample_url !== ""
  )
    data.email_sample_url = body.email_sample_url;
  if (
    body.email_sample !== undefined &&
    body.email_sample !== null &&
    body.email_sample !== ""
  )
    data.email_sample = body.email_sample;
  if (
    body.pdf_sample !== undefined &&
    body.pdf_sample !== null &&
    body.pdf_sample !== ""
  )
    data.pdf_sample = body.pdf_sample;
  if (
    body.email_proof !== undefined &&
    body.email_proof !== null &&
    body.email_proof !== ""
  )
    data.email_proof = body.email_proof;
  if (
    body.pdf_url !== undefined &&
    body.pdf_url !== null &&
    body.pdf_url !== ""
...
const save_trade_file_log = async (req: any, body: any) => {
  ...
  if (
    body.original_file_name !== undefined &&
    body.original_file_name !== null &&
    body.original_file_name !== ""
  )
    file_log.original_file_name = body.original_file_name;
  if (
    body.output_file_name !== undefined &&
    body.output_file_name !== null &&
    body.output_file_name !== ""
  )
    file_log.output_file_name = body.output_file_name;
...
const update_pre_trade_file_log = async (req: any, body: any) => {
  ...
  if (
    ...
    data.original_file_name = body.original_file_name;
  if (
    body.output_file_name !== undefined &&
    body.output_file_name !== null &&
    body.output_file_name !== ""
  )
    data.output_file_name = body.output_file_name;
  if (body.status !== undefined && body.status !== null && body.status !== "")
    data.status = body.status;

  const results = await clientTradeModel.update_pre_trade_file_log(
    data,
    body.pre_tades_file_id
  );
