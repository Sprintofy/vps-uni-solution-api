...
const save_trades_by_client = async(req:any)=> {
    try {
        ...
        req.body.pdf_sample = await emailNotificationServiceService.generatePreTradePdfSampleFile(1,req.body)
...
const fetch_trades_details_by_client_id = async(req:any)=> {
    try {
        ...
        const trades = await clientTradeModel.fetch_trade_proof_details(req.query.proof_id)
...
const save_pre_trade_proofs = async(req:any,body:any)=> {
    ...
    if (body.pdf_sample !== undefined && body.pdf_sample !== null && body.pdf_sample !== '') data.pdf_sample = body.pdf_sample;
