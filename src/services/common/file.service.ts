...
import fileStorageService from "../utilities/fileStorageService"; // ADD THIS
  ...
  return new Promise((resolve, reject) => {
    upload(req, req.res, (err: any) => {
      ...
      for (const key in req.body) {
        normalizedFields[key] = Array.isArray(req.body[key])
          ? req.body[key][0]
          : req.body[key];
      }
...
//     const s3FolderPath =
//       CONSTANTS.AWS.S3_BUCKET.FOLDER_NAME +
//       "/organization_" +
//       organization_id +
//       "/pdfs";
...
//     const result = await awsS3Bucket.uploadFile(
//       fileStream,
//       s3FolderPath,
//       body.file_name
//     );
...
const uploadPdfFileToS3Bucket = async (organization_id: any, body: any) => {
  try {
    ...
    // This will be: organization_1/emails/filename.pdf
...
const uploadSampleEmailPdfFileToS3Bucket = async (
  ...
  try {
    ...
    console.error(`Error uploading sample email PDF: ${error.message}`);
    throw error;
  }
};
...
const uploadEmailFileToS3Bucket = async (organization_id: any, body: any) => {
  try {
    ...
    console.error(`Error uploading email file: ${error.message}`);
    throw error;
  }
};
