...
class ClientTradeModel extends BaseModel {
  ...
  async fetchAllClientsTradesByOrganization(
    ...
    filter_data && filter_data.is_email_received == 1
      ? (query += " AND ptp.is_email_received = 1 ")
      : "";
    filter_data && filter_data.is_email_received == -1
      ? (query += " AND ptp.is_email_received = 0 ")
      : "";
    ...
    search_text !== undefined && search_text !== null && search_text !== ""
      ? ((query += "  AND ( c.client_name LIKE ? OR c.client_code LIKE ? ) "),
  ...
  async fetchAllClientsProofsStatistics(
    ...
    filter_data && filter_data.is_email_received == 1
      ? (query += " AND pft.is_email_received = 1 ")
      : "";
    filter_data && filter_data.is_email_received == -1
      ? (query += " AND pft.is_email_received = 0 ")
      : "";
  ...
  async fetch_all_trade_proof_urls() {
    const query = `SELECT
    pre_trade_proof_id, client_code, created_date,
    CASE 
      WHEN pdf_url IS NULL THEN NULL
      WHEN pdf_url LIKE 'http%' THEN pdf_url
      WHEN pdf_url LIKE 'proofs/%' THEN 
